...
public class DangerousMaterialsFilter implements PathFilter {
     ...
     * around each node.
     *
     * @param dangerousMaterials The list of materials to avoid, e.g., List.of(Material.LAVA, Material.CACTUS).
     ...
     */
    @Override
    public boolean filter(@NotNull PathValidationContext context) {
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int y = -radius; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
                    ...
                    if (block != null && dangerousMaterials.contains(block.getBlockInformation().getMaterial())) {
